...
def play_game(token, user_agent=None):
    url = "https://game-domain.blum.codes/api/v2/game/play"  # Changed to v2
    headers = get_headers(token, user_agent)
    max_retries = 3
    for attempt in range(max_retries):
        try:
...
def claim_game(token, game_id, points, dogs, user_agent=None):
    url = "https://game-domain.blum.codes/api/v2/game/claim"
    headers = get_headers(token, user_agent)
    payload = create_payload(game_id, points, dogs)
    if payload is None:
        log_error("Failed to create payload for claiming game.")
        return 0
...
def auto_play_game(token, user_agent=None, game_points_min=131, game_points_max=210):
    ...
    while True:
        ...
        if game_id:
            ...
            dogs = random.randint(25, 30) * 5  # Assuming dogs is a random value similar to points
            reward = claim_game(token, game_id, points, dogs, user_agent=user_agent)
            total_reward += reward
...
def main():
    ...
    while True:
        ...
        if user_choice == '1':
       include_auto_play = False  # Disable auto game play by default
       ...
       # auto_play_game(token, user_agent=user_agent, game_points_min=game_points_min, game_points_max=game_points_max)
       print(f"{Fore.RED + Style.BRIGHT}Auto game play is disabled.{Style.RESET_ALL}")
